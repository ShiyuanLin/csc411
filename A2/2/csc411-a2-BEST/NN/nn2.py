...
def ReLUBackward(grad_y, x, y):
    ...
    ###########################
    # Insert your code here.
    grad_x = np.multiply(grad_y, y > 0.0)
    return grad_x
    ###########################
    raise Exception('Not implemented')
...
def NNUpdate(model, eps, momentum):
    ...
    dW1, dW2 ,dW3 = np.zeros(model['W1'].shape), np.zeros(model['W2'].shape), np.zeros(model['W3'].shape)
    db1, db2 ,db3 = np.zeros(model['b1'].shape), np.zeros(model['b2'].shape), np.zeros(model['b3'].shape)
        
    
    dW1 = momentum * dW1 - eps * model['dE_dW1']
    dW2 = momentum * dW2 - eps * model['dE_dW2']
    dW3 = momentum * dW3 - eps * model['dE_dW3']  
    db1 = momentum * db1 - eps * model['dE_db1']
    db2 = momentum * db2 - eps * model['dE_db2']
    db3 = momentum * db3 - eps * model['dE_db3']
